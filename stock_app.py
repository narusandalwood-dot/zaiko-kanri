...
# ==========================================
def render_barcode_scanner(label_target, widget_key, button_text="バーコードスキャン", button_color="#FF4B4B"):
    """
    label_target: 値を流し込む対象の text_input の label名
    widget_key: Streamlitの内部状態(key)を書き換えるためのID
    button_text: ボタンに表示する文字
    button_color: ボタンの色
    """
    import streamlit.components.v1 as components
    
    # 🌟 JavaScriptの { } を Pythonの f-string 用に {{ }} へエスケープ
    scan_js = f"""
            ...
            try {{
                ...
                const scan = async () => {{
                    ...
                    if (barcodes.length > 0) {{
                        ...
                        // 🌟 2. 重要：Streamlitの内部状態に値を「確定」させて送る
                        // ここを {{widget_key}} にして widget_key 引数の中身を流し込む
...
# 1. 検索ラベルを定義（JSが探す目印）
    ...
    # 🌟 3. スキャナー関数を呼び出す
    # label_target に加えて、widget_key（今の名札）も教える
    render_barcode_scanner(
        label_target=search_label, 
        widget_key=current_key, # ← これを追加！
        button_text="📷 バーコード読み取り開始", 
        button_color="#FF4B4B"
    )
